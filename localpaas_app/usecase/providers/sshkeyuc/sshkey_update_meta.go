package sshkeyuc

import (
	"context"

	"github.com/localpaas/localpaas/localpaas_app/apperrors"
	"github.com/localpaas/localpaas/localpaas_app/base"
	"github.com/localpaas/localpaas/localpaas_app/basedto"
	"github.com/localpaas/localpaas/localpaas_app/infra/database"
	"github.com/localpaas/localpaas/localpaas_app/pkg/bunex"
	"github.com/localpaas/localpaas/localpaas_app/pkg/timeutil"
	"github.com/localpaas/localpaas/localpaas_app/pkg/transaction"
	"github.com/localpaas/localpaas/localpaas_app/usecase/providers/sshkeyuc/sshkeydto"
)

func (uc *SSHKeyUC) UpdateSSHKeyMeta(
	ctx context.Context,
	auth *basedto.Auth,
	req *sshkeydto.UpdateSSHKeyMetaReq,
) (*sshkeydto.UpdateSSHKeyMetaResp, error) {
	err := transaction.Execute(ctx, uc.db, func(db database.Tx) error {
		sshkeyData := &updateSSHKeyData{}
		err := uc.loadSSHKeyDataForUpdateMeta(ctx, db, req, sshkeyData)
		if err != nil {
			return apperrors.Wrap(err)
		}

		uc.prepareUpdatingSSHKeyMeta(req, sshkeyData)
		return uc.persistSSHKeyMeta(ctx, db, sshkeyData)
	})
	if err != nil {
		return nil, apperrors.Wrap(err)
	}

	return &sshkeydto.UpdateSSHKeyMetaResp{}, nil
}

func (uc *SSHKeyUC) loadSSHKeyDataForUpdateMeta(
	ctx context.Context,
	db database.IDB,
	req *sshkeydto.UpdateSSHKeyMetaReq,
	data *updateSSHKeyData,
) error {
	setting, err := uc.settingRepo.GetByID(ctx, db, base.SettingTypeSSHKey, req.ID, false,
		bunex.SelectFor("UPDATE OF setting"),
	)
	if err != nil {
		return apperrors.Wrap(err)
	}
	if req.UpdateVer != setting.UpdateVer {
		return apperrors.Wrap(apperrors.ErrUpdateVerMismatched)
	}
	data.Setting = setting

	return nil
}

func (uc *SSHKeyUC) prepareUpdatingSSHKeyMeta(
	req *sshkeydto.UpdateSSHKeyMetaReq,
	data *updateSSHKeyData,
) {
	timeNow := timeutil.NowUTC()
	setting := data.Setting
	setting.UpdateVer++
	setting.UpdatedAt = timeNow

	if req.Status != nil {
		setting.Status = *req.Status
	}
	if req.ExpireAt != nil {
		setting.ExpireAt = *req.ExpireAt
	}
}

func (uc *SSHKeyUC) persistSSHKeyMeta(
	ctx context.Context,
	db database.IDB,
	data *updateSSHKeyData,
) error {
	err := uc.settingRepo.Update(ctx, db, data.Setting,
		bunex.UpdateColumns("update_ver", "updated_at", "status", "expire_at"),
	)
	if err != nil {
		return apperrors.Wrap(err)
	}
	return nil
}
